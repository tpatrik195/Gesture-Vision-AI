import { DEFAULT_MAPPINGS, isSupportedAction } from "../shared/actions.js";
...
async function getSettings() {
  const data = await chrome.storage.local.get(["gestureEndpoint", "mappings", "minConfidence"]);
  return {
...
chrome.runtime.onInstalled.addListener(async () => {
  const current = await chrome.storage.local.get(["gestureEndpoint", "mappings", "minConfidence"]);
  const seed = {};
